...
**Implementation note (#4, 2026-06-21):** A codex profile takes over the center pane the same way file view does (it unmounts `ChapterStackManager`; `CodexContext.activeCodexEntry` is the routing flag). `CodexNotesEditor` is a fresh `LexicalComposer` that deliberately omits `EditorRefPlugin` — the notes surface must NOT publish itself into `EditorContext`, or it would clobber the chapter editor ref the outline/jump plugins depend on. Clicking a mention closes the profile (`openCodexEntry(null)`) then calls `useJumpToOccurrence`; the module-level jump registry (#38) decouples the timing so the request survives the `ChapterStackManager` remount. Snippet highlight slicing uses `Array.from` (code points) to line up with Rust's char-based `snippetMatchStart/Len`.
...
| Codex UI | Index panel + profile done (#4); data model (#36), scan (#37), jump (#38) done | Entity-profile + manuscript-backlink index. #36/#37/#38/#4 complete. #4: CodexPanel.tsx (sidebar navigator, categories → entries → lazy mention-count badges, per-category create), CodexProfile.tsx (center pane: header + editable alias chips + notes region + Mentions rail), CodexNotesEditor.tsx (self-contained Lexical rich-text, no global EditorRef hijack). CodexContext holds activeCodexEntry; center pane routes to CodexProfile (mirrors file view); right tool-sidebar yields to icon strip while a profile is open. Left-pane codex entries now also open profiles. Remaining: #39 (search panel), #40 (define-from-selection); deferred #41/#42/#43. See "Codex as a manuscript index" decision. |
...
### Codex frontmatter: omit when default, emit when non-default (2026-06-19)
`save_codex_entry` writes a `---` YAML frontmatter block only when there is something non-default to record (non-empty aliases OR `matchCase: false`). Files with no aliases and default matchCase are stored as plain Markdown with no frontmatter, keeping them clean. The parser treats any file not starting with `---\n` as a no-frontmatter file with defaults (`aliases: []`, `matchCase: true`). No external YAML crate — frontmatter is hand-parsed with a minimal line scanner.
