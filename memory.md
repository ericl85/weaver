...
| Theming | Backend complete (#34); panel pending (#7) | CSS-variable tokens + raw-CSS theme files (Obsidian-style). Done: #10 (tokens + Default + editor wiring), #6 (`.weaver/` dir + migration), #33 (chrome zinc sweep), #34 (Rust theme commands). Pending: #7 (theme manager panel). `themes.rs`: `list_themes` (scans `.weaver/themes/*/manifest.json`, skips malformed, `[]` if dir missing — Default is not a folder), `read_theme_css` (fresh read, no cache), `get_active_theme`/`set_active_theme` (read-modify-write raw JSON map in `config.json` to preserve other keys; `null` = Default). Theme name validated against path traversal; folder name is the lookup key. Frontend wrappers in `src/lib/tauri.ts`; `ThemeManifest` replaced old `Theme` in types. Old `Theme` object scrapped. |
